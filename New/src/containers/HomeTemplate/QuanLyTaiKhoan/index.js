      ...
      const response = await axios.post('http://localhost:5000/delete-account', {
        data: {
          id: accountToDelete.id,
           // Gửi ID của tài khoản cần xóa
        },
      });
      console.log(response.data); // Kết quả từ API

      // Xóa tài khoản khỏi danh sách
      setAccountList((prevList) => prevList.filter((account) => account.id !== accountToDelete.id));
    ...
    <div style={{ paddingLeft: '40px', paddingRight: '40px' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          ...
          >
            <h4 style={{ color: '#FFFFFF', textAlign: 'center', cursor: 'pointer' }}>
              Nội bộ nhân viên
            </h4>
          </div>
          <div
          ...
          <tbody>
            {isLoading ? (
              ...
              accountList.data.map((account, index) => (
                <tr key={account.id} style={index === accountList.length - 1 ? lastRowStyle : null}>
